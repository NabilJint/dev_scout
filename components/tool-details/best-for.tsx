...
// Icon map for common user types
...
function getTagIcon(tag: string): ReactElement {
  const lower = tag.toLowerCase()
  return tagIcons[lower] || (
    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    <section className="rounded-xl border border-border-subtle bg-surface-elevated p-6">
      ...
      <div className="flex flex-wrap gap-3">
        {tags.map((tag, index) => (
          <div
            key={index}
            className="flex items-center gap-2 rounded-lg border border-border-subtle bg-n-700 px-3 py-2 text-sm text-text-secondary"
          >
            {getTagIcon(tag)}
            <span>{tag}</span>
          </div>
